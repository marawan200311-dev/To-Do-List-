...
from instance.databas import initialise_data, add_data, show_up,delete,update
...
def delete_page():
    data=show_up()
    return render_template('delete.html', schedule=data)
...
def update_index():
    data=show_up()
    return render_template('update.html',data=show_up())
@app.route('/update_new_assignment' , methods=['POST'])
def update_data():
    id  = request.form['id']
    new_assignment = request.form['assignment']
    new_date =  request.form['date']
    update(id,new_assignment,new_date)
    return render_template('updat.html')
@app.route('/skip_update')
def skip_button():
    return redirect('/')
