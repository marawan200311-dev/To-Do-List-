...
def add_data(assignment,data):
   connect = sqlite3.connect('data.db')
   cursor= connect.cursor()
   cursor.execute('INSERT INTO user_assignment (assignment,date) VALUES(?,?)', (assignment,data))
   connect.commit()
   connect.close()
...
def update(assignment,date):
  connect = sqlite3.connect('data.db')
  cursor= connect.cursor()
  cursor.execute(" UPDATE user_assignment SET () WHERE Country='Mexico' ")
